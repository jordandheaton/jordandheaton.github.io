...
def main() -> int:
    ...
    def render(stamp: str) -> str:
        ...
        return ("/* AUTO-GENERATED by scraper/generate_data.py -- do not hand-edit.\n"
                f"   Source: data/catalog.json | generated {stamp} */\n"
                "const CATALOG_DATA = " + json.dumps(payload, ensure_ascii=False) + ";\n")

    js = render(str(date.today()))
    # Keep the previous date when nothing else changed, so an unchanged catalog
    # produces a byte-identical file. Otherwise the date alone is a real diff:
    # the scheduled refresh would commit and redeploy the site every week even
    # in weeks BYU published no edits, and the git history would stop showing
    # when the catalog actually moved.
    if OUT_PATH.exists():
        prev = OUT_PATH.read_text(encoding="utf-8")
        m = re.search(r'"generated": "(\d{4}-\d{2}-\d{2})"', prev)
        if m and render(m.group(1)) == prev:
            js = prev
            print(f"catalog unchanged since {m.group(1)} -- keeping that date "
                  f"so {OUT_PATH.name} stays byte-identical")
