...
try:
    ...
    # left off (leaf, frame ~507, on the very first scene of the run) -- so the
    # opening frames of "hook" read as leaf/tree-canopy regardless of `a` while
    # the playhead catches up, which is what makes the open read as a grounded,
    # recognizable start rather than a hard cut. `a`=0.455 lands the fast-travel
    # target at the Planet-Earth chapter (blue-marble Earth against black, the
    # most recognizable single image the footage has -- there is no literal
    # cityscape in the baked frames, see task-6-report.md); `b`=0.985 lands deep
    # in the Milky Way chapter, one chapter shy of the terminal full-galaxy
    # frame. Confirmed by reading f00000/f00150/f00299 after capture.
    for name, frames, a, b, ease in (
        ("hook", 300, 0.455, 0.985, ease_io),    # 5s fast leaf->earth->galaxy scrub
